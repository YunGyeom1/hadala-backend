...
@router.post("/", response_model=schemas.RetailShipmentResponse)
def create_retail_shipment(
    ...
    return crud.create_retail_shipment(db, shipment_create, current_profile.id)

@router.get("/search", response_model=schemas.PaginatedResponse)
def search_retail_shipments(
    search_params: schemas.RetailShipmentSearch = Depends(),
    current_profile: Profile = Depends(get_current_profile),
    db: Session = Depends(get_db)
):
    """
    ...
    shipments, total = crud.search_retail_shipments(db, search_params, current_profile.company_id)
...
@router.get("/{shipment_id}", response_model=schemas.RetailShipmentResponse)
...
@router.get("/{shipment_id}/items", response_model=List[schemas.RetailShipmentItemResponse])
def get_shipment_items(
    ...
    return crud.get_retail_shipment_items(db, shipment_id)

@router.put("/{shipment_id}", response_model=schemas.RetailShipmentResponse)
def update_retail_shipment(
    ...
    updated_shipment = crud.update_retail_shipment(db, shipment_id, shipment_update)
    return updated_shipment

@router.patch("/{shipment_id}/status", response_model=schemas.RetailShipmentResponse)
def update_shipment_status(
    ...
    """
    check_shipment_access(db, shipment_id, current_profile, require_ownership=True)
    updated_shipment = crud.update_retail_shipment_status(db, shipment_id, status_update)
    return updated_shipment

@router.patch("/{shipment_id}/payment-status", response_model=schemas.RetailShipmentResponse)
def update_payment_status(
    ...
    """
    check_shipment_access(db, shipment_id, current_profile, require_ownership=True)
    updated_shipment = crud.update_retail_shipment_payment_status(db, shipment_id, status_update)
    return updated_shipment
...
def delete_retail_shipment(
    ...
    """
    check_shipment_access(db, shipment_id, current_profile, require_ownership=True)
    if not crud.delete_retail_shipment(db, shipment_id):
        raise HTTPException(
...
def delete_shipment_item(
    ...
    """
    check_shipment_access(db, shipment_id, current_profile, require_ownership=True)
    if not crud.delete_retail_shipment_item(db, shipment_id, item_id):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
