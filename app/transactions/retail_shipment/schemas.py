...
class RetailShipmentItemResponse(RetailShipmentItemBase):
    id: UUID4
    shipment_id: UUID4
    total_price: float
...
class RetailShipmentUpdate(RetailShipmentBase):
    title: Optional[str] = None
    items: Optional[List[RetailShipmentItemCreate]] = None

class RetailShipmentResponse(RetailShipmentBase):
    id: UUID4
    creator_id: UUID4
    total_price: float
    shipment_status: ShipmentStatus
    payment_status: PaymentStatus
    items: List[RetailShipmentItemResponse]

    class Config:
        from_attributes = True

class RetailShipmentSearch(BaseModel):
    title: Optional[str] = None
...
class PaginatedResponse(BaseModel):
    items: List[RetailShipmentResponse]
    total: int
    page: int
    page_size: int
    total_pages: int
    ...
    @classmethod
    def create(cls, items: List[RetailShipmentResponse], total: int, page: int, page_size: int) -> "PaginatedResponse":
