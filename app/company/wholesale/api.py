...
@router.get("/{company_id}/inventory-snapshots/{snapshot_date}", response_model=schemas.CompanyInventorySnapshot)
def get_company_inventory_snapshot(
    ...
    # 스냅샷 조회 (없으면 자동 생성)
    snapshot = crud.get_company_inventory_snapshot(
        db=db,
    ...
    if not snapshot:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="해당 날짜의 재고 스냅샷을 찾을 수 없습니다"
        )
